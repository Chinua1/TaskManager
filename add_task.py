...
class AddTaskPage( webapp2.RequestHandler ):
    def get( self, board_key, board_index ):
        ...
        template = JINJA_ENVIRONMENT.get_template( 'pages/add_task.html' )
    ...
    def post( self, board_key, board_index ):
        ...
        if cta_button == 'Add Task':
            if task_title == '' or due_date == '':
                ...
                url = "/boards/" + board_key + "/" + board_index + "/add-task" + query_string
                self.redirect( url )
                return
            else:
                ...
                if self.isTaskNameUniqueOnBoard(tasks, task_title):
                    new_task = Task(
                        title = task_title,
                        due_date = due_date,
                        completed = False,
                    ...
                    message = "Task title already exists on board. Change to proceed"
